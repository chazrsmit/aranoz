          ...
          <div className="d-flex align-items-center user-section">
            {auth.user ? (
              auth.user.image ? (
                ...
                <div className="user-avatar" style={{ backgroundColor: 'white' }} />
              )
            ) : null}

            <div className="nav-item dropdown user-dropdown ms-2">
              <a className="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                {auth.user?.name}
              </a>
              <ul className="dropdown-menu">
                <li><Link href={route('logout')} method="post" className="dropdown-item">Log out</Link></li>
                <li><Link href={route('home')} className="dropdown-item">Back home</Link></li>
              </ul>
            </div>
          </div>
        </div>
